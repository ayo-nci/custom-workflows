...
export default defineComponent({
  async run({ steps, $ }) {
    ...
    const update = async (json, id) => {
      ...
      const req_url =
        hub_url + object_type + "/batch/update/?" + "hapikey=" + "13df";
    ...
    if (steps.check_deal_on_hubspot.$return_value[0] === "End") {
      console.log("Exiting...");
    } else if (steps.check_deal_on_hubspot.$return_value[0] !== "Not found") {
      var upd_obj = {};
      ...
      upd_obj["last_sync_date"] = steps.check_deal_on_hubspot.$return_value[1];
      upd_obj["sync_status"] = false;

      console.log("Updating on hubspot");
      var update_deal = await update(
        upd_obj,
        steps.check_deal_on_hubspot.$return_value[0]
      );
      if (typeof Number(update_deal) == "number") {
